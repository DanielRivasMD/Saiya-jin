...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  {:des "Hyper Mode"
   ...
   [;
    ; arrow glyphs
    ^{:doc/actions [{:program c/zj,    :action "page up",            :exec ["MoveTab \"Left\";"]}]}    [(c/mk c/ihyper c/al) [(c/mk c/b c/s c/pu)] :term]
    ^{:doc/actions [{:program c/zj,    :action "page down",          :exec ["MoveTab \"Right\";"]}]}   [(c/mk c/ihyper c/ar) [(c/mk c/b c/s c/pd)] :term]
    ^{:doc/actions [{:program c/zj,    :action "home",               :exec ["Resize \"Increase\";"]}]} [(c/mk c/ihyper c/au) [(c/mk c/b c/s c/hm)] :term]
    ^{:doc/actions [{:program c/zj,    :action "end",                :exec ["Resize \"Decrease\";"]}]} [(c/mk c/ihyper c/ad) [(c/mk c/b c/s c/ed)] :term]
    ...
    ; action glyphs
    ^{:doc/actions [{:program c/zj, :action "pane float pin",    :exec ["TogglePanePinned;"]}]}                            [(c/mk c/ihyper c/db) [:!SOm] :term]
    ^{:doc/actions [{:program c/zj, :action "pane float toggle", :exec ["ToggleFloatingPanes;"]}]}                         [(c/mk c/ihyper c/re) [:!SOn] :term]
    ^{:doc/actions [{:program c/zj, :action "mode search",       :exec ["SwitchToMode \"EnterSearch\"; SearchInput 0;"]}]} [(c/mk c/ihyper c/rs) [:!Oh]  :term]
    ^{:doc/actions [{:program c/zj, :action "pane float pop",    :exec ["TogglePaneEmbedOrFloating;"]}]}                   [(c/mk c/ihyper c/ro) [:!SOl] :term]
    ^{:doc/actions [{}]} [(c/mk c/ihyper c/rc)   [(c/mk c/ohyper c/rc)]]
    ^{:doc/actions [{:program c/zj, :action "mode lock",         :exec ["SwitchToMode \"Locked\";"]}
                    {:program c/zj, :action "mode normal",       :exec ["SwitchToMode \"Normal\";"]}]}                     [(c/mk c/ihyper c/sp) [:!Og]  :term]
