...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  {:des "Option - Command Mode"
   ...
   [;
  ; arrow glyphs
    ^{:doc/actions [{:program c/zj, :action "jump prev buffer", :exec ["GoToPreviousTab;"]}]}    [:!OC#Pleft_arrow          [:!SOj]             :term]
    ^{:doc/actions [{:program c/zj, :action "jump next buffer", :exec ["GoToNextTab;"]}]}    [:!OC#Pright_arrow         [:!SOk]             :term]
    ^{:doc/actions [{:action "scroll up", :exec ["scroll_up"], :program "helix-common"}
                    {:action "scroll up", :exec ["scrollUpMain-alt2"], :program "lazygit"}]}    [:!OC#Pup_arrow            [:!Tx]              :term]
    ^{:doc/actions [{:action "scroll down", :exec ["scroll_down"], :program "helix-common"}
                    {:action "scroll down", :exec ["scrollDownMain-alt2"], :program "lazygit"}]}    [:!OC#Pdown_arrow          [:!Ty]              :term]
  ...
  ; technical glyphs
    ^{:doc/actions [{:action "tab sync",  :exec ["ToggleActiveSyncTab;"], :program c/zj}]}    [:!OC#Popen_bracket  [:!SOv] [:term]]
    ^{:doc/actions [{:action "plugin jump-list",  :exec ["LaunchOrFocusPlugin \"file:~/.config/zellij/plugins/zellij-jump-list.wasm\" { floating true; move_to_focused_tab true; };"], :program c/zj}]}    [:!OC#Pclose_bracket [:!SOw] [:term]]
    ^{:doc/actions [{:action "plugin monocle",   :exec ["LaunchOrFocusPlugin \"file:~/.config/zellij/plugins/monocle.wasm\" { floating true; }; SwitchToMode \"Normal\";"], :program c/zj}]}    [:!OC#Psemicolon     [:!SOx] [:term]]
  ...
  ; action glyphs
    ^{:doc/actions [{:action "tab close",     :exec ["CloseTab;"], :program c/zj}]} [:!OC#Pdelete_or_backspace [:!SOu] [:term]]
    ^{:doc/actions [{:action "plugin room",      :exec ["LaunchOrFocusPlugin \"file:~/.config/zellij/plugins/room.wasm\" { floating true; ignore_case true; };"], :program c/zj}]} [:!OC#Preturn_or_enter     [:!SOp] [:term]]
    ^{:doc/actions [{:action "tab new",       :exec ["NewTab;"], :program c/zj}]} [:!OC#Pright_shift         [:!SOq] [:term]]
    ^{:doc/actions [{:action "tab break pane",:exec ["BreakPane;"], :program c/zj}]} [:!OC#Pright_option        [:!SOr] [:term]]
    ^{:doc/actions [{:program c/zj,    :action "mode tab rename",    :exec zj-rename-tab-mode}
                    {:program c/zt,    :action "abort tab reanme",   :exec zt-abort-rename}]}      [:!OC#Pright_command  [:!SOs] :term]
    ^{:doc/actions [{:action "tab jump back", :exec ["ToggleTab;"], :program c/zj}]} [:!OC#Pspacebar            [:!SOt] [:term]]
  ...
  ; numeric glyphs
    ^{:doc/actions [{:action "alt-cmd-1", :exec ["SwitchToMode \"Tab\"; GoToTab 1; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P1 [:!SOa] [:term]]
    ^{:doc/actions [{:action "alt-cmd-2", :exec ["SwitchToMode \"Tab\"; GoToTab 2; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P2 [:!SOb] [:term]]
    ^{:doc/actions [{:action "alt-cmd-3", :exec ["SwitchToMode \"Tab\"; GoToTab 3; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P3 [:!SOc] [:term]]
    ^{:doc/actions [{:action "alt-cmd-4", :exec ["SwitchToMode \"Tab\"; GoToTab 4; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P4 [:!SOd] [:term]]
    ^{:doc/actions [{:action "alt-cmd-5", :exec ["SwitchToMode \"Tab\"; GoToTab 5; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P5 [:!SOe] [:term]]
    ^{:doc/actions [{:action "alt-cmd-6", :exec ["SwitchToMode \"Tab\"; GoToTab 6; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P6 [:!SOf] [:term]]
    ^{:doc/actions [{:action "alt-cmd-7", :exec ["SwitchToMode \"Tab\"; GoToTab 7; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P7 [:!SOg] [:term]]
    ^{:doc/actions [{:action "alt-cmd-8", :exec ["SwitchToMode \"Tab\"; GoToTab 8; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P8 [:!SOh] [:term]]
    ^{:doc/actions [{:action "alt-cmd-9", :exec ["SwitchToMode \"Tab\"; GoToTab 9; SwitchToMode \"Normal\";"], :program c/zj}]} [:!OC#P9 [:!SOi] [:term]]
    ^{:doc/actions [{:action "alt-cmd-0", :exec ["GoToPreviousTab;"], :program c/zj}]} [:!OC#P0 [:!SOj] [:term]]
